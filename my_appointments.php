...
<body>
<?php
// Add notification and message counts for sidebar badges
$unread_messages = 0;
...
if ($_SESSION['user_type'] === 'parent') {
    try {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) as unread_count FROM messages WHERE recipient_id = ? AND status = 'pending'");
        $stmt->bind_param("i", $_SESSION['user_id']);
        $stmt->execute();
        $unread_messages = $stmt->get_result()->fetch_assoc()['unread_count'];
