...
try {
    switch ($action) {
        ...
        case 'add_common_symptom':
            addCommonSymptom($conn, $input);
            break;

        case 'update_common_symptom':
            updateCommonSymptom($conn, $input);
            break;

        case 'delete_common_symptom':
            deleteCommonSymptom($conn, $input);
            break;
...
function restoreBackup($conn, $input) {
    ...
    $command = sprintf('%s -h %s -u %s %s < %s 2>&1', $mysqlcmd, 'localhost', 'root', 'baby_tracker1', escapeshellarg($path));
...
function addCommonSymptom($conn, $input) {
    ...
    $query = "INSERT INTO common_symptoms (symptom_name, description, age_range_min_months, age_range_max_months, home_remedies, when_to_see_doctor, created_at)
              VALUES (?, ?, ?, ?, ?, ?, NOW())";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('ssiiis', $name, $desc, $min_age, $max_age, $remedies, $when_doc);
    if ($stmt->execute()) {
...
function updateCommonSymptom($conn, $input) {
    ...
    $query = "UPDATE common_symptoms SET symptom_name=?, description=?, age_range_min_months=?, age_range_max_months=?, home_remedies=?, when_to_see_doctor=? WHERE id=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('ssiiisi', $name, $desc, $min_age, $max_age, $remedies, $when_doc, $id);
    if ($stmt->execute()) {
