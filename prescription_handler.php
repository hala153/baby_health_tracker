...
// For write operations (add, renew), restrict to doctors/nurses
$action = $_POST['action'] ?? $_GET['action'] ?? null;
if (in_array($action, ['add_prescription', 'renew_prescription']) && !in_array($_SESSION['user_type'], ['doctor', 'nurse'])) {
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => 'Non authorized']);
    exit;
}
...
// 4. Obtaining specific recipe details
    ...
    // Check if parent is accessing their own child's prescription
    if ($_SESSION['user_type'] === 'parent') {
        ...
        $check_stmt->bind_param('ii', $prescription_id, $_SESSION['user_id']);
        $check_stmt->execute();
        if ($check_stmt->get_result()->num_rows === 0) {
            die(json_encode(['success' => false, 'message' => 'غير مصرح بالوصول لهذه الوصفة']));
        }
        $check_stmt->close();
    }
